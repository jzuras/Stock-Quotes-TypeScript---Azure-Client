//#region imports
import { getStockQuote } from './modules/getStockQuote.js';
...
const refreshInterval = 10000; // API has limit of 8 calls/min
 ...
 */
function toLocalFormat(dateString) {
    try {
...
function showInChartButtonClick(row) {
    ...
    companyStockDataPromise
        .then((data) => {
        if (data.error == false)
            drawChart(canvasChart, data.stockData);
        else {
            divChart.style.display = 'none';
